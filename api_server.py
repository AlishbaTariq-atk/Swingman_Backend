...
# --- Configuration ---
# Use environment variables for production-readiness. Default to 'output'.
...
class SessionEndResponse(BaseModel):
    ...
    payload: FinalArtifacts

class APISwingSession:
    """ Manages the state and logic for a single, headless swing analysis session. """
    def __init__(self, session_name="api_session"):
        print("Initializing new API swing session...")
    ...
    def finalize_current_swing(self) -> Dict[str, Any]:
        ...
        if not metrics:
            return {"error": "Swing analysis failed to produce metrics."}
        ...
        if swing_data.get("impact_point"):
            self.heatmap_generator.add_impact_point(point=swing_data["impact_point"])
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        print("Closing connection and cleaning up session.")
        await asyncio.to_thread(session.end_session_and_get_artifacts)
        await websocket.close()

if __name__ == "__main__":
    import uvicorn
    # For production, Uvicorn is typically run by a process manager like Gunicorn.
    # For local running, this is fine.
